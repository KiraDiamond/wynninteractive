...
import { GENERATED_MOB_MARKERS as EXISTING_MOB_MARKERS } from "../data/generated-mob-markers.js";
import { MOBS_CONTENT as EXISTING_MOB_CONTENT } from "../data/content/mob-content.js";
...
const OUTPUT_ROOT = path.join(ROOT, "data", "refresh-cache", "mob-drops");
...
const CONTENT_OUTPUT_PATH = path.join(ROOT, "data", "content", "mob-content.js");
const COUNTS_OUTPUT_PATH = path.join(ROOT, "data", "generated-mob-counts.js");
...
function buildMobImageIndex(existingMarkers) {
  const exact = new Map();
  ...
  for (const marker of existingMarkers) {
    const image = normalizeWhitespace(marker.iconImage);
    const name = normalizeWhitespace(stripFormattingCodes(marker.title));
    if (!image || !name) {
      continue;
    }
...
function buildMarkersModule(markers) {
  return [
    ...
    "",
    `export const GENERATED_MOB_MARKERS=${JSON.stringify(markers)};`,
    "",
  ].join("\n");
}
...
function buildContentModule(content) {
  return [
    "// Generated by scripts/build-mob-markers.mjs",
    "",
    `export const MOBS_CONTENT = ${JSON.stringify(content, null, 2)};`,
    "",
  ].join("\n");
}

function mergeMarkers(existingMarkers, refreshedMarkers) {
  const refreshedById = new Map(refreshedMarkers.map((marker) => [marker.id, marker]));
  const merged = existingMarkers.map((marker) => {
    const refreshed = refreshedById.get(marker.id);
    if (!refreshed) {
      return marker;
    }
    refreshedById.delete(marker.id);
    return {
      ...marker,
      ...refreshed,
      iconImage: refreshed.iconImage || marker.iconImage || "",
    };
  });
  return [...merged, ...refreshedById.values()].sort((left, right) => left.title.localeCompare(right.title));
}

function buildCountsModule(markers) {
  const counts = Object.fromEntries(
    MOB_FAMILY_DEFINITIONS.map((family) => [
      family.id,
      markers.filter((marker) => marker.category === family.id).length,
    ]),
  );
  return [
    "// Generated by scripts/build-mob-markers.mjs",
    "",
    `export const GENERATED_MOB_CATEGORY_COUNTS = ${JSON.stringify(counts, null, 2)};`,
    "",
  ].join("\n");
}
...
async function main() {
  ...
  const mobImageIndex = buildMobImageIndex(EXISTING_MOB_MARKERS);
  ...
  const refreshedContent = Object.fromEntries(contentEntries);
  const mergedMarkers = mergeMarkers(EXISTING_MOB_MARKERS, markers);
  const content = { ...EXISTING_MOB_CONTENT, ...refreshedContent };

  await fs.writeFile(MARKERS_OUTPUT_PATH, buildMarkersModule(mergedMarkers), "utf8");
  await fs.writeFile(CONTENT_OUTPUT_PATH, buildContentModule(content), "utf8");
  await fs.writeFile(COUNTS_OUTPUT_PATH, buildCountsModule(mergedMarkers), "utf8");
  await fs.writeFile(SUMMARY_PATH, buildSummaryMarkdown(items, mobs, mergedMarkers), "utf8");

  console.log(JSON.stringify({
    ingredients: items.filter((item) => item.type === "ingredient").length,
    refreshedMobs: markers.length,
    totalMobs: mergedMarkers.length,
    preservedMobs: mergedMarkers.length - markers.length,
