...
import { WIKI_MAP_MARKERS } from "./data/wiki-map-markers.js?v=20260520e";
...
function categoryCount(categoryId) {
  return state.markers.filter((marker) => marker.category === categoryId && !marker.fixed && !marker.contextOnly && markerArea(marker) === state.currentArea).length;
}

function categoryVisibleCount(categoryId) {
  return state.filteredMarkers.filter((marker) => marker.category === categoryId && !marker.fixed && !marker.contextOnly && markerArea(marker) === state.currentArea).length;
}
